...
class TestGraphqlResolvers:
    ...
    def test_list_resolver_returns_objects(self):
    ...
    def test_single_resolver_returns_object(self):
        from demo.models import Product
        from snapadmin.api.graphql import schema
...
class TestUrlsGraphqlFailure:
    def test_graphql_wiring_failure_branch(self, capsys):
        import snapadmin.urls as snap_urls
        from graphene_django.views import GraphQLView

        # GRAPHQL_ENABLED defaults to True, so reloading enters the try block.
        # Force GraphQLView.as_view to raise to exercise the except/print branch.
        try:

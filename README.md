...
- [`es_filter()`](https://drofji.github.io/django-snapadmin/#es-filter) structured term filters, [`es_aggregate()`](https://drofji.github.io/django-snapadmin/#es-aggregate) facets, [`es_count()`](https://drofji.github.io/django-snapadmin/#es-count) true match totals past the search limit, [`es_scan()`](https://drofji.github.io/django-snapadmin/#es-scan) deep scan past the 10k window (`source=False` streams just the pks of N-million matches, `limit=N` bounds the walk) — each with a DB fallback when ES is off, or [`db_fallback=False`](https://drofji.github.io/django-snapadmin/#es-db-fallback) to fail loudly instead on a table the DB can't scan
- [Resumable bulk reindex](https://drofji.github.io/django-snapadmin/#bulk-reindex-command) — `snapadmin_reindex` pages by a `pk__gt` cursor with live progress, `--resume`, `--parallel`, and `--tune`; it fetches only the ES-mapped columns, and `--limit N` bounds a probe run
...
- [Pluggable export sources](https://drofji.github.io/django-snapadmin/#export-sources) — register a custom row source with [`SNAPADMIN_EXPORT_SOURCES`](https://drofji.github.io/django-snapadmin/#export-sources) (ES-query-backed, key-list, or a custom document shape) while the runner keeps its resumable chunking, progress, cancel and storage
...
- [Feature-adoption audit](https://drofji.github.io/django-snapadmin/#snapadmin-info) — `snapadmin_info --section features` prints a ✓/✗ checklist of which commerce-important capabilities (backups, retention-based deletion, PII masking, audit, read-only models, rate limiting, SSO, …) are actually on or in use in your project vs. left unused

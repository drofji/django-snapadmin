...
# The Unfold-detection block and the admin-generation methods that used to
# live here moved to snapadmin.admin_gen (#SIMPL1f); UNFOLD_INSTALLED and
# _wysiwyg_widget are imported back below since formatted_id() (and a test)
...
# SnapJobBase, SnapExportJob, SnapReindexJob and SnapImportJob moved to
# snapadmin.jobs (#SIMPL1f) and are imported below, near the top of this
# file, alongside this module's other snapadmin.* imports — re-exported here
# so every existing ``from snapadmin.models import SnapExportJob`` etc. is
# unaffected. They stay part of this app's model state (Django resolves
# app_label from the containing app config, not the specific submodule), and
# this import runs while snapadmin.models is loading, which is when Django's
# app registry expects models to register.
...
class SnapModel(AdminGenMixin, models.Model):
    ...
    * **Compliance** — ``data_retention_days``/``data_retention_files`` for the
    ...
    # register_all_admins() moved to snapadmin.admin_gen's AdminGenMixin
